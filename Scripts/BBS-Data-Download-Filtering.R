...
if(!(dir.exists("./Data/BBS_raw"))) {
  dir.create("./Data/BBS_raw")
}
...
grab_bbs_data(sb_id = "5ea04e9a82cefae35a129d65", bbs_dir = "./Data/BBS_raw")

# Delete unnecessary files to save storage space. Retain metadata files for later
# use.

file.remove("./Data/BBS_raw/50-StopData.zip")
file.remove("./Data/BBS_raw/MigrantNonBreeder.zip")

# Decompress ZIP files to access annual total species counts for all routes in
# British Columbia.

zip::unzip("./Data/BBS_raw/States.zip", exdir = "./Data/BBS_raw/")
zip:unzip("./Data/BBS_raw/States/BritCol.zip", exdir = "./Data/BBS_raw/British_Columbia/")

# Remove unused States to save storage space. Retain Washington and Oregon for 
# use in later analyses

states_delete <- list.files(path = "./Data/BBS_raw/States", full.names = TRUE)
states_delete <- states_delete[!(states_delete %in% paste0("./Data/BBS_raw/States/", c("BritCol.zip", "Washing.zip", "Oregon.zip")))]

file.remove(states_delete)

# Read and filter British Columbia total annual species counts and filter to
# Point Grey route (#212).
PointGrey_TotalCounts <- read_csv("./Data/BBS_raw/British_Columbia/BritCol.csv") %>%
  filter(Route == 212)
...
if(!(dir.exists("./Data/BBS_filtered"))) {
  dir.create("./Data/BBS_filtered")
}
